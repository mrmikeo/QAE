			...
			if (vendorData && vendorData.qae1)
			{
				...
				if (contractData.tp == 'GENESIS' && transactionData.recipient == QaeMasterAddress)
				{
					...
					try 
					{
						...
						const vTransactionOutput = implement(QaeTransactionOutput)(TransactionOutput);
					...
					try 
					{
						...
						const vTransactionDetails = implement(QaeTransactionDetails)(TransactionDetails);
					...
					try 
					{
						...
						const vTokenStats = implement(QaeTokenStats)(TokenStats);
					...
					try 
					{
						...
						const vTokenObject = implement(QaeTokenObject)(TokenObject);
					...
					try 
					{
						...
						const vAddressObject = implement(QaeAddressObject)(AddressObject);
					...
					try 
					{
						...
						const vTransactionObject = implement(QaeTransactionObject)(TransactionObject);
				...
				else if (contractData.tp == 'MINT' && transactionData.recipient == QaeMasterAddress)
				{
					...
					(async () => {
						...
						var findToken = await qdb.findDocument('tokens', {"tokenIdHex": tokenId});
					...
					if (failed === false)
					{
						...
						(async () => {
							...
							var findAddress = qdb.findDocument('addresses', {"recordId": recordId});
							if (findAddress == null)
							{
							...
							var newBalance = Big(findAddress.tokenBalance).plus(mintAmount);
							...
							var xqrspent = Big(findtoken.tokenStats.qty_xqr_spent).plus(transactionData.amount);
							
							await qdb.updateDocument('addresses', '{"recordId":"' + recordId + '"}', '{"tokenBalance":"' + newBalance.toString() + '"}');
							
							await qdb.updateDocument('tokens', '{"tokenDetails.tokenId":"' + tokenId + '"}', '{"lastUpdatedBlock": ' + blockData.height + ', "tokenStats.block_last_active_mint": ' + blockData.height + ', "tokenStats.qty_valid_txns_since_genesis": ' + newValidTxns + ', "tokenStats.qty_token_minted":"' + totalMinted.toString() + '", "tokenStats.qty_token_circulating_supply":"' + circSupply.toString() + '", "tokenStats.qty_xqr_spent":"' + xqrspent.toString() + '"}');
				...
				else if (contractData.tp == 'SEND')
				{
					...
					if (failed === false)
					{
						...
						(async () => {
							...
							var findSenderAddress = qdb.findDocument('addresses', {"recordId": srecordId});
							...
							var newSenderBalance = Big(findSenderAddress.tokenBalance).minus(sendAmount);
							...
							var findRecipientAddress = qdb.findDocument('addresses', {"recordId": rrecordId});
							if (findSenderAddress == null)
							{
							...
							else 
							{
								...
								var newRecipientBalance = Big(findRecipientAddress.tokenBalance).plus(sendAmount);
							...
							await qdb.updateDocument('tokens', '{"tokenDetails.tokenId":"' + tokenId + '"}', '{"lastUpdatedBlock": ' + blockData.height + ', "tokenStats.block_last_active_send": ' + blockData.height + ', "tokenStats.qty_valid_txns_since_genesis": ' + newValidTxns + ', "tokenStats.qty_valid_token_addresses": ' + newTokenAddrs + ', "tokenStats.qty_xqr_spent":"' + xqrspent.toString() + '"}');
				...
				else if (contractData.tp == 'BURN' && transactionData.recipient == QaeMasterAddress)
				{
					...
					if (failed === false)
					{
						...
						(async () => {
							...
							await qdb.updateDocument('tokens', '{"tokenDetails.tokenId":"' + tokenId + '"}', '{"lastUpdatedBlock": ' + blockData.height + ', "tokenStats.qty_valid_txns_since_genesis": ' + newValidTxns + ', "tokenStats.qty_token_burned":"' + totalBurned.toString() + '", "tokenStats.qty_token_circulating_supply":"' + circSupply.toString() + '", "tokenStats.qty_xqr_spent":"' + xqrspent.toString() + '"}');
