...
var qaeDB = /** @class */ (function () 
{
	...
	/* This is mainly used for Journal entries */
	qaeDB.prototype.insertDocumentIncrement = function(collectionName, query) 
	{

		autoIncrement.getNextSequence(this.db, collectionName, function (err, autoIndex) {
	...
	qaeDB.prototype.createJournalEntry = function(txid, blockId, blockHeight, timestamp, timestampUnix, action, collectionName, fieldData, recordData)
	{
		...
		return new Promise((resolve, reject) => {
			...
			this.insertDocumentIncrement('journal', insertData).then(value => {
				// fulfillment
			
				if (value == 1) // This is the first entry
				{
			
					// chainHash = recordHash

					this.updateDocument('journal', {"_id": value }, {"chainHash": recordHash }).then(uresult => {
				...
				else
				{
					...
					var previousId = Big(value).minus(1).toFixed(0);
			
					this.findDocument('journal', {'_id': previousId}).then(prevdoc => {
			
						var chainHash = SparkMD5.hash(prevdoc.recordHash . '' . recordHash);

						this.updateDocument('journal', {"_id": value }, {"chainHash": chainHash }).then(uresult => {
						...
						console.log("JOURNAL FIND ERROR!!!");
				...
				console.log("JOURNAL ENTRY ERROR!!!");
